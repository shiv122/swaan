...
class TemplateCustomizer {
  constructor({
    ...
    this.settings.defaultNavbarFixed =
      typeof defaultNavbarFixed !== "undefined"
        ? defaultNavbarFixed
        : DEFAULT_NAVBAR_FIXED;
    this.settings.defaultFooterFixed =
      typeof defaultFooterFixed !== "undefined"
        ? defaultFooterFixed
        : DEFAULT_FOOTER_FIXED;

    this.settings.availableThemes =
      availableThemes || TemplateCustomizer.THEMES;
    ...
    if (this.settings.styles.length < 2) {
      ...
      if (i !== -1) {
        this.settings.controls = this.settings.controls
          .slice(0, i)
          .concat(this.settings.controls.slice(i + 1));
      }
    }
  ...
  setLayoutType(pos, updateStorage = true) {
    ...
    if (
      this.settings.layoutType === "fixed" ||
      this.settings.layoutType === "fixed-offcanvas"
    ) {
      // Set perfectscrollbar wheelPropagation false for fixed layout
      if (PerfectScrollbarLib && menuScroll) {
        window.Helpers.menuPsScroll.destroy();
        menuScroll = new PerfectScrollbarLib(
  ...
  setLang(lang, force = false) {
    ...
    const tt = t.themes || {};
    const themes =
      this.container.querySelectorAll(".template-customizer-theme-item") || [];
  ...
  update() {
    ...
    const hasHorizontalMenu = !!document.querySelector(
      ".layout-menu-horizontal.menu, .layout-menu-horizontal .menu"
    );
    const isLayout1 = !!document.querySelector(
  ...
  _loadSettings() {
    ...
    if (
      lType !== "" &&
      ["static", "static-offcanvas", "fixed", "fixed-offcanvas"].indexOf(
        lType
      ) !== -1
    ) {
      type = lType;
    ...
    this.settings.rtl =
      rtl !== "" ? rtl === "true" : this.settings.defaultTextDir;

    this.settings.style =
      this.settings.styles.indexOf(style) !== -1
        ? style
    ...
    if (!this._hasControls("style"))
      this.settings.style = cl.contains("dark-style") ? "dark" : "light";
  ...
  _setup(_container = document) {
    ...
    if (this.settings.displayCustomizer)
      customizerW.setAttribute("style", "visibility: visible");
    else customizerW.setAttribute("style", "visibility: hidden");

    // Open btn
    //
    const openBtn = this.container.querySelector(
    ...
    if (!this._hasControls("style")) {
      ...
      if (this.settings.style === "dark")
        style.setAttribute("checked", "checked");
    ...
    if (!this._hasControls("themes")) {
      ...
      const themesWInner = themesW.querySelector(
      ...
      themesWInner
        .querySelector(`input[value="${this.settings.theme.name}"]`)
    ...
    const themingW = this.container.querySelector(
    ...
    if (
      ...
      const layoutTypeW = this.container.querySelector(
      ...
      if (!this._hasControls("layoutType")) {
        ...
        this._controls.layoutType = layoutTypeW.querySelector(
      ...
      if (!this._hasControls("showDropdownOnHover")) {
        ...
        this._controls.showDropdownOnHover = this._controls.showDropdownOnHover.querySelector(
          "input"
        );
        ...
        const showDropdownOnHover = (e) =>
          this.setDropdownOnHover(e.target.checked);
      ...
      if (!this._hasControls("layoutNavbarFixed")) {
        ...
        this._controls.layoutNavbarFixed = this._controls.layoutNavbarFixedW.querySelector(
          "input"
        );
        ...
        const layoutNavbarFixedCb = (e) =>
          this.setLayoutNavbarFixed(e.target.checked);
      ...
      if (!this._hasControls("layoutFooterFixed")) {
        ...
        this._controls.layoutFooterFixed = this._controls.layoutFooterFixedW.querySelector(
          "input"
        );
        ...
        const layoutFooterFixedCb = (e) =>
          this.setLayoutFooterFixed(e.target.checked);
  ...
  _initStyle() {
    ...
    */
    ...
    const classesToRemove =
      style === "light" ? ["dark-style"] : ["light-style"];
  ...
  _insertStylesheet(className, href) {
    ...
    if (
      ...
      document.write(
        `<link rel="stylesheet" type="text/css" href="${href}" class="${className}">`
      );
    }
  ...
  _loadingState(enable, themes) {
    this.container.classList[enable ? "add" : "remove"](
      `template-customizer-loading${themes ? "-theme" : ""}`
    );
  }
  ...
  _setSetting(key, val) {
    ...
    try {
      localStorage.setItem(
        `templateCustomizer-${layoutName}--${key}`,
        String(val)
      );
    } catch (e) {
      // Catch Error
  ...
  _removeListeners() {
    for (let i = 0, l = this._listeners.length; i < l; i++) {
      this._listeners[i][0].removeEventListener(
        this._listeners[i][1],
        this._listeners[i][2]
      );
    }
